...
function isHiddenStudioModel(m) {
  const hay = `${m?.id || ''} ${m?.name || ''} ${m?.endpoint || ''} ${m?.description || ''}`.toLowerCase();
  return hay.includes('watermark-remover') || /remove watermarks/.test(hay);
}
...
export async function getStudioModels(category) {
  ...
  // Only expose models we can run with verified static metadata (endpoint, fields, controls).
  filtered = filtered.filter((m) => m.hasStaticMeta && !isHiddenStudioModel(m));
...
/** Look up live (or cached) pricing for a model id / endpoint slug. */
export async function getModelPricing(modelId) {
  ...
  try {
    ...
    const hit =
      all.find((m) => m.id === id) ||
      all.find((m) => m.endpoint === id) ||
      all.find((m) => m.endpoint?.endsWith(`/${id}`));
    if (hit) {
      return {
        id: hit.id,
        costUsd: hit.costUsd,
        costCredits: hit.costCredits,
        dynamicPricing: hit.dynamicPricing,
      };
    }
  } catch {
    /* fall through */
...
export function getStaticFallback(category) {
  ...
  const list = (map[category] || []).filter((m) => !isHiddenStudioModel(m));
  return {
    category,
    models: list.map((m) => ({
      id: m.id,
