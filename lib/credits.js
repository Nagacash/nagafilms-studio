...
/**
 * Model-aware credit estimate (live MuAPI catalog when available).
 * Falls back to DEFAULT_IMAGE_USD_COST × markup.
 */
export async function estimateModelCredits(modelOrEndpoint) {
  const fallbackUsd = Number(process.env.DEFAULT_IMAGE_USD_COST || 0.04);

  if (modelOrEndpoint) {
    try {
      const { getModelPricing } = await import('./model-catalog');
      const priced = await getModelPricing(modelOrEndpoint);
...
/** @deprecated use estimateModelCredits */
